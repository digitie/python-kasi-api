...
from kasi import KasiAuthError, KasiClient
...
def _async_live_client() -> KasiClient:
    if os.getenv("KASI_LIVE") != "1":
        pytest.skip("set KASI_LIVE=1 to run live KASI data.go.kr tests")
    try:
        return KasiClient.from_env(timeout=15, retries=1)
...
@pytest.mark.live
async def test_live_holidays_from_env_key() -> None:
...
@pytest.mark.live
async def test_live_solar_to_lunar() -> None:
    async with _live_client() as client:

        page = (await client.solar_to_lunar(sol_year=2026, sol_month=5, sol_day=7))
...
@pytest.mark.live
async def test_live_area_rise_set() -> None:
    async with _live_client() as client:

        page = (await client.area_rise_set(locdate="20260507", location="서울"))
